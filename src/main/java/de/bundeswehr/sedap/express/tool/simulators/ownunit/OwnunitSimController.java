...
public class OwnunitSimController {
	...
	@FXML
	private ComboBox<CommandState> commandStateComboBox;
	...
	void initialize() {
		...
		commandStateComboBox.setItems(FXCollections.observableArrayList(CommandState.values()));

		Callback<ListView<CommandState>, ListCell<CommandState>> cellFactory = _ -> new ListCell<>() {
			@Override
			protected void updateItem(CommandState item, boolean empty) {
				...
				case Partially_executed_successfully -> {
					setText("Partially executed successfully");
				}
				case Executed_not_successfully -> {
					setText("Executed unsuccessfully");
				}
				case Execution_not_possible -> {
					setText("Execution not possible");
				}
				case Will_execute_at -> {
					setText("Will execute at");
				}
				case Undefined -> {
					setText("Undefined");
				}
		...
		// Sowohl für das Dropdown-Menü als auch das Haupt-Textfeld setzen:
		commandStateComboBox.setCellFactory(cellFactory);
		commandStateComboBox.setButtonCell(cellFactory.call(null));
	...
	public static Position calculateNewPosition(Position startPosition, double azimuthDegrees, double distanceMeters) {
		...
		// 1. Erdradius definieren (Standard-Erdradius in Metern, z. B. WGS84
		// Äquatorradius 6.378.137 m)
		...
		// 3. Distanz in ein Angle-Objekt (Bogenlänge auf der Kugel) umrechnen: Radians
		// = Distanz / Radius
